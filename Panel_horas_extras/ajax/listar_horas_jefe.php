...
$fecha_inicial_carga = $_POST['fecha_ini'];

$fecha_fin_carga = $_POST['fecha_fin'];
$fecha_fin_carga_2 = date("Y-m-d", strtotime($fecha_fin_carga . "+ 1 days"));

// if ($_SESSION['intranet_id'] == 479) {
if ($_SESSION['intranet_id'] == 479) {
    $id = 881;
...
$query = mysqli_query($con, "SELECT $campos FROM  $tables WHERE id_jefe ='$id'  AND `fecha_registro` BETWEEN '$fecha_inicial_carga' AND '$fecha_fin_carga_2' ");

$query_pendiente = mysqli_query($con, "SELECT $campos FROM  $tables WHERE id_jefe ='$id'  AND `fecha_registro` BETWEEN '$fecha_inicial_carga' AND '$fecha_fin_carga_2' AND `estado_jefe` = 0");

$query_aprobado = mysqli_query($con, "SELECT $campos FROM  $tables WHERE id_jefe ='$id'  AND `fecha_registro` BETWEEN '$fecha_inicial_carga' AND '$fecha_fin_carga_2' AND `estado_jefe` = 3 ");
?>
<div class="row">
    <div class="col-1">
        <h3>Total</h3>
    ...
    <div class="col-2">
        <h3>Pendiente</h3>
        <?php echo mysqli_num_rows($query_pendiente); ?>
    </div>
    <div class="col-1">
        <h3>Aprobado</h3>
        <?php echo mysqli_num_rows($query_aprobado); ?>
    </div>
</div>
<?php
unset($_SESSION['horas_actuales_jefe']);
?>
<!doctype html>
<html lang="en">
                    ...
                    <table id="example" class="table table-hover" cellspacing="0">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <font size="2" , color="#2d2d2d">FECHA DE REGISTRO
                                </th>
                                <th>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <font size="2" , color="black"><?php echo $fechaR; ?>
                                    </td>
                                    <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#example tfoot .fecha').each(function() {
                var titles = $(this).text();
                $(this).html('<input type="date" class="form-control" placeholder="Búsqueda"' + titles + '"/>');
            });
            $('#example tfoot .estado').each(function() {
                var titles = $(this).text();
        ...
        function ver(id) {
            var url = "./ajax/listar_Detalle_jefe.php"
            var id = id;
            $.ajax({
                type: "POST",
                url: url,
