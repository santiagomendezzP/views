<div class="modal fade" id="aprobar_con_vice" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color: #d0d0d0;">
        <h3 class="modal-title" id="exampleModalLabel">APROBAR</h3>
      ...
      <div class="modal-body">
        ...
        <input type="hidden" id="correo_solicitante_vice" name="correo_solicitante_vice">
        <input type="hidden" id="nombre_solicitante_vice" name="nombre_solicitante_vice">
        <br>
        <label>Si</label>
...
if(isset($_REQUEST['aceptar_co_j'])){
  $correo_solicitante = $_REQUEST['correo_solicitante_vice'];
  $nombre_solicitante = $_REQUEST['nombre_solicitante_vice'];
  $id_re = $_REQUEST['id_r'];
  require("conexion.php");
    if($_REQUEST['option']==3){ 
      $sql2="UPDATE `consolidado_horas` SET `estado_vice` = '3' WHERE `id_reporte` = '$id_re'";  
      $query = mysqli_query($con,$sql2);
      $sql3="UPDATE `reporte_horas` SET `estado_vice` = '3' WHERE `id_reporte` = '$id_re'";  
      $query = mysqli_query($con,$sql3);

    }else{
      $sql2="UPDATE `consolidado_horas` SET `estado_vice` = '4' WHERE `id_reporte` = '$id_re'";  
      $query = mysqli_query($con,$sql2);
      $sql3="UPDATE `reporte_horas` SET `estado_vice` = '4' WHERE `id_reporte` = '$id_re'";  
      $query = mysqli_query($con,$sql3);
    ...
    echo "<script type='text/javascript'>window.location='reporte_horas_vice.php';</script>";
  }
?>