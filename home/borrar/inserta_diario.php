                ...
                <ul class="site-menu-sub">
                  ...
                  <li class="site-menu-item" id="site_medio_ambiente">
                    <a class="animsition-link" href="../ambiental/?<?php echo mt_rand(); ?>">
                      <span class="site-menu-title" style="font-family:'Lucida Sans Unicode', 'Lucida Grande', sans-serif">Cuidamos el planeta</span>
                    </a>
                  </li>
...
<  <div class="page">
 ...
 <?php 
  ...
  if(isset($_POST['guardar'])){
    $temperatura_ini=$_POST['temperatura_ini'];
    $tos=$_POST['tos'];
    ...
    $salud_cambio=$_POST['salud_cambio'];
    $jornada=$_POST['jornada'];
   
$intranet_documento=$_SESSION['intranet_documento'];
$intranet_name=$_SESSION['intranet_name'];
...
if ($temperatura_ini >= 37.5 ) {
	$estado = "TIENE FIEBRE REPORTA DE INMEDIATO";

}



    if(!empty($temperatura_ini)  && !empty($tos) && !empty($d_respirar) && !empty($fatiga) && !empty($malestar) && !empty($secreciones) && !empty($dolor_g) && !empty($olfato)  && !empty($salud_cambio)  &&  !empty($intranet_documento) && !empty($intranet_name) && !empty($intranet_usuario) &&  !empty($intranet_id)  &&  !empty($intranet_cargo)  &&  !empty($intranet_proyecto) &&  !empty($jornada) && !empty($estado)){

        $consulta_insert=$con->prepare('INSERT INTO test_diario(temperatura_ini,tos,d_respirar,fatiga,malestar,secreciones,dolor_g,olfato,salud_cambio,cedula,nombre,usuario,id_usuario,cargo,proyecto,fecha,estado,jornada) VALUES(:temperatura_ini,:tos,:d_respirar,:fatiga,:malestar,:secreciones,:dolor_g,:olfato,:salud_cambio,:intranet_documento,:intranet_name,:intranet_usuario,:intranet_id,:intranet_cargo,:intranet_proyecto,NOW(),:estado,:jornada)');
        $consulta_insert->execute(array(



          ':temperatura_ini' =>$temperatura_ini,
          ':tos' =>$tos,
          ...
          ':dolor_g' =>$dolor_g,
          ':olfato' =>$olfato,
          ':salud_cambio' =>$salud_cambio,
          ':intranet_documento' =>$intranet_documento,
...
<script>
     swal("Gracias", "Gracias por diligenciar la encuesta :)", "success");
    </script>
