...
// }
    ...
    if(!empty($tos) && !empty($fiebre) &&  !empty($d_respirar) && !empty($fatiga) && !empty($malestar) && !empty($secreciones) && !empty($dolor_g) && !empty($olfato)  && !empty($salud_cambio)  && !empty($grupo_r)  && !empty($grupo_r_no)  &&  !empty($intranet_perfil)  &&     !empty($usuario) &&  !empty($intranet_usuario)  &&  !empty($intranet_cargo)  &&  !empty($intranet_proyecto) &&  !empty($jornada) && !empty($estado)){

        $consulta_insert=$con->prepare('INSERT INTO encuesta_diaria(tos,fiebre,d_respirar,fatiga,malestar,secreciones,dolor_g,olfato,salud_cambio,grupo_r,grupo_r_no, usuario,id_usuario,id_cargo,id_proyecto,id_perfil,fecha,estado,tapa_bocas,jornada) VALUES(:tos,:fiebre,:d_respirar,:fatiga,:malestar,:secreciones,:dolor_g,:olfato,:salud_cambio,:grupo_r,:grupo_r_no,:usuario,:intranet_usuario,:intranet_cargo,:intranet_proyecto,:intranet_perfil,NOW(),:estado,:tapa_b,:jornada)');
